    ...
    >
      {value?.map((item, index) => (
        <Badge
          key={item}
          className={cn(
            "mx-0.5 my-0.5 flex items-center gap-1 rounded-xl px-1",
            activeIndex === index && "ring-2 ring-muted-foreground "
          )}
          variant={"secondary"}
        >
          <span className="text-xs">{capitalizeFirstLetter(item)}</span>
          <button
